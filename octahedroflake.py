...
def slicer_work(stl_file, config_file):
    # https://help.prusa3d.com/article/list-of-placeholders_205643
    #
    slicer_path = '/Applications/Prusa/PrusaSlicer.app/Contents/MacOS/PrusaSlicer'

    # https://onlinepngtools.com/convert-png-to-base64
    with open('thumnail-6.txt', 'r', encoding='utf-8') as file:
        thumnail_data = file.read()

    base_action = f'{slicer_path} {stl_file}'
    slicing_params = (
        " --spiral-vase"
        " --perimeters 1"
        " --fill-density 0"
    ...
    notes = f'''
    downloed from https://www.printables.com/model/432767
    gcode generated by Nat a Cyborg, please be nice.
    Say hi if you see this!
    https://www.printables.com/social/6773-nat-a-cyborg/about
    ...
    output_params = (
        f" --load {config_file}"
        f" --output-filename-format 'Octahedroflake-{FINAL_ORDER}_{FULL_HEIGHT}mm_{{printer_model}}_{{initial_filament_type}}_{{print_time}}.gcode' -g"
        )

    action = f"{base_action}{slicing_params} --notes '{notes.strip()}'{output_params}"
    os.system(action)
...
def run():
    ...
    # for subdir, dirs, files in os.walk(printerConfigs):
    #     for file in files:
    #         config_file = os.path.join(subdir, file)
    #         stl_file = f'{directory}/{name}.stl'
    #         slicer_work(stl_file, config_file)
